...
- Donut 的启动/停止单元格按其 UIA 默认动作执行；标记为双击的控件会在主窗口置前后执行物理双击。
...
- BitBrowser 或 Donut 本地名称为 `地区-类型-序号`、中央名称为 `地区-类型-节点-序号` 时，只有中央存在唯一候选才会自动匹配（例如 `TH-BT-10` → `TH-BT-HK05-10`、`GB-DT-07` → `GB-DT-HK3-07`）；
